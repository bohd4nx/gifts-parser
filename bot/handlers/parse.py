...
from bot.utils.texts import get_status_message, get_final_message, INIT_TEXT
    ...
    def get_keyboard(is_parsing: bool = True, chat: str = None) -> InlineKeyboardMarkup:
        if is_parsing:
            return InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="⛔️ Остановить парсинг", callback_data="stop_parse")
            ]])
        if chat:
            return InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="📥 Скачать результаты", callback_data=f"download_{chat}")
            ]])
        return None
...
async def stop_parse_callback(callback_query: CallbackQuery, state: FSMContext):
    if not parse_manager.active_session:
        await callback_query.answer("❌ Нечего останавливать", show_alert=True)
        return
    ...
    try:
        ...
        if session.total_members == 0:
            await status_msg.edit_text(
                "❌ Не удалось получить количество участников. Убедитесь, что группа публичная и доступная.")
            return
...
async def download_results(callback_query: CallbackQuery):
    ...
    if not file_path or not os.path.exists(file_path):
        await callback_query.answer("Результаты не найдены", show_alert=True)
        return
    ...
    try:
        ...
        await callback_query.answer("Ошибка при загрузке результатов", show_alert=True)
